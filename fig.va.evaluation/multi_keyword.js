...
const relabels = alphanum_range("A", "Z");
...
$(function () {
  ...
  var author_name = "Eduard Gröller";
  ...
  var animation_frame = getParameterByName("animation_frame");
  ...
  $("#startYear").text(year_from);
  $("#currentYear").text(year_from + animation_frame);
  $("#endYear").text(year_to);
  ...
  var labelFontSizeScale = d3.scale.log().range([16, 20]);
  var labelOpacityScale = d3.scale.log().range([1, 1]);
  ...
  linkWidthScale = d3.scale.linear().range([0, 10]);
  ...
  d3.csv(url, function (error, rawdata) {
    ...
    linkWidthScale.domain([0, preferences["seq_max"]]);
  ...
  $("#advance").on("click", function () {
    if (animation_frame < year_to - year_from) {
      animation_frame++;
    }
  ...
  $("#play").on("click", function () {
    var interval = setInterval(function () {
      ...
      // .transition(
      //   d3.transition().attrTween("stroke-width", function () {
      //     return d3.interpolateNumber(
      //       linkWidthScale(d.seq[animation_frame - 1]),
      //       linkWidthScale(d.seq[animation_frame])
      //     );
      //   })
      // );
    }, 1000);
