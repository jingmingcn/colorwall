...
$(function () {
  ...
  if (!keyword) keyword = "";
  if (!year_from) year_from = 2009;
  if (!year_to) year_to = 2016;
  if (!threshold) threshold = 0;
  if (!node_r_min) node_r_min = 8;
  if (!node_r_max) node_r_max = 16;
  if (!link_distance) link_distance = 100;
  if (!force_charge) force_charge = -800;
  ...
  d3.json(url, function (error, rawdata) {
    ...
    preferences["year_from"] = 2009;

    preferences["year_to"] = 2016;
    ...
    mdata.nodes.forEach((node) => {
      var n = getNodeOrCreate(node.id);
      n.value = 1;
    });

    mdata.links.forEach((link) => {
      link.seq[0].forEach((i) => {
    ...
    mdata.links.forEach((link) => {
      var node1 = getNodeOrCreate(link.source);
      var node2 = getNodeOrCreate(link.target);
    ...
    edges.forEach((e) => {
      var v = 0;
      e.seq[0].forEach((i) => (v += i));
      e.seq[1].forEach((i) => (v += i));
      e.value = v;
    });
  ...
  var app = function () {
      ...
      .each(function (d, i) {
        ...
        d3.select(this)
          ...
          .attr("fill", function (d, i) {
            ...
            return sequentialScale(d / seq_max_0);
          })
    ...
    path1 = container
      ...
      .each(function (d, i) {
        ...
        d3.select(this)
          ...
          .attr("height", function (d) {
            return (
              d3.min([nodeRadiusScale(ld.source), nodeRadiusScale(ld.target)]) /
              2
            );
          })
          .attr("fill", function (d, i) {
            //return color(d / preferences["seq_max"]);
            return sequentialScale(d / seq_max_1);
          })
          .style("opacity", function (d, i) {
            if (d == 0) {
              return 0.5;
    ...
    layout_folder
      .add(preferences, "toggle_treemap")
      .name("Toggle TreeMap")
      .onFinishChange(function (value) {
        uri = window.location.href;
    ...
    layout_folder
      .add(preferences, "link_distance", 50, 200)
      .step(10)
      .name("Link Distance")
      .onFinishChange(function (value) {
    ...
    layout_folder
      .add(preferences, "node_radius_min", 1, 10)
      .step(1)
      .name("Node R. Min")
      .onFinishChange(function (value) {
        if (value != node_r_min) {
    ...
    layout_folder
      .add(preferences, "node_radius_max", 10, 40)
      .step(1)
      .name("Node R. Max")
      .onFinishChange(function (value) {
        if (value != node_r_max) {
    ...
    dataset_folder
      .add(preferences, "year_from", 1990, 2016)
      .step(1)
      .name("(Year) From")
      .listen()
      .onFinishChange(function (value) {
    ...
    dataset_folder
      .add(preferences, "year_to", 1990, 2016)
      .step(1)
      .name("(Year) To")
      .listen()
      .onFinishChange(function (value) {
    ...
    dataset_folder
      .add(preferences, "seq_threshold", 1, 20)
      .step(1)
      .name("(Seq) Threshold")
      .listen()
      .onFinishChange(function (value) {
    ...
    dataset_folder
      .add(preferences, "link_filter", 1, 20)
      .step(1)
      .name("Link Filter")
      .listen()
      .onFinishChange(function (value) {
    ...
    path.each(function (d, i) {
      ...
      d3.select(this)
        ...
        .each(function (s, i) {
          ...
          if (area_width < 0) {
            console.log(dr, source_r, target_r);
          }
