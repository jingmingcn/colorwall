...
$(function () {
  ...
  if (!force_charge) force_charge = -100;
  ...
  d3.csv(url, function (error, rawdata) {
    ...
    function getNodeOrCreate(t,aff) {
      var node;
      if (!nodesMap.has(t)) {
        nodesMap.set(t, { id: t, name: t, value: 0,aff: aff });
      }
      return nodesMap.get(t);
    }
    ...
    function addCount(t,aff) {
      var node = getNodeOrCreate(t,aff);
      node.value += 1;
      nodesMap.set(t, node);
      return node;
    }
    ...
    mdata.forEach(function (d) {
      ...
      var author_aff = d["Author Affiliations"].split(";");
      var year = new Number(d["Year"]);

      author.forEach(function (d,i) {
        if(author_aff[i]){
          addCount(d,author_aff[i]);
        }else{
          addCount(d);
        }
