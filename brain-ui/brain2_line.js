...
$(function(){
	...
	if (!force_charge)	force_charge	= 	-20;
	...
	var	preferences = {
		...
		'window_step': parseInt(window_step),
		'filter_threshold': parseInt(filter_threshold)
	};
	...
	var app = function(){
		...
		dataset_folder.add(preferences, 'adhd_sample',adhd_samples).name('ADHD Sample').onFinishChange(function(value){
			if(value!=adhd_sample){
				...
				window.location.href = updateQueryStringParameter(uri, 'adhd_sample', value);
			}
		});
		...
		dataset_folder.add(preferences, 'tdc_sample',tdc_samples).name('TDC Sample').onFinishChange(function(value){
			if(value!=tdc_sample){
				uri = window.location.href;
				window.location.href = updateQueryStringParameter(uri, 'tdc_sample', value);
			}
		});
		...
		dataset_folder.add(preferences, 'frame_of_reference', ['ADHD','TDC']).name('Frame of Ref.').onFinishChange(function(value){
			if(value!=frame_of_reference){
				uri = window.location.href;
				window.location.href = updateQueryStringParameter(uri, 'frame_of_reference', value);
			}
		});
		...
		dataset_folder.add(preferences, 'window_size',10,50).step(10).name('Window Size').onFinishChange(function(value){
			if(value!=window_size){
				uri = window.location.href;
				window.location.href = updateQueryStringParameter(uri, 'window_size', value);
			}
		});
		...
		dataset_folder.add(preferences, 'window_step',5,50).step(5).name('Window Step').onFinishChange(function(value){
			if(value!=window_step){
				uri = window.location.href;
				window.location.href = updateQueryStringParameter(uri, 'window_step', value);
			}
		});

		dataset_folder.add(preferences, 'filter_threshold',1,100).step(1).name('Filter Threshold').onFinishChange(function(value){
			if(value!=filter_threshold){
				uri = window.location.href;
				window.location.href = updateQueryStringParameter(uri, 'filter_threshold', value);
			}
		});
